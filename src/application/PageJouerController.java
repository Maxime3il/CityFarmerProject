/**
*	La classe PageJouerController est un contrôleur qui gère la vue du jeu.
*	Elle gère le mouvement du personnage, la gestion des interactions avec les objets,
*	l'affichage des informations du personnage et son inventaire, la gestion du son.
*/
...
public class PageJouerController {
	...
	/**
	 * Charge un fichier FXML et affiche la fenêtre correspondante.
	 *
	...
	/**
	 * Configure les touches du clavier pour permettre le mouvement du personnage et
	 * les interactions avec différents éléments du jeu.
	 */
	...
	/**
	 * Initialise les écouteurs pour les touches de déplacement et la variable
	 * d'animation.
	 */
	...
	/**
	 * AnimationTimer qui gère les interactions de collision du sprite avec les
	 * objets du jeu et la mise à jour de sa position sur la scène.
	 */
	AnimationTimer timer = new AnimationTimer() {
		@Override
		public void handle(long timestamp) {
			// Interaction avec le marchand
			if (sprite.getLayoutX() <= 1328 && sprite.getLayoutX() >= 1222 && sprite.getLayoutY() <= 580
					&& sprite.getLayoutY() >= 370) {
				BoutonInteraction.setVisible(true);
			} else {
				BoutonInteraction.setVisible(false);
			}
			// Interaction avec le potager
			if ((sprite.getLayoutX() <= 960 && sprite.getLayoutX() >= 784 && sprite.getLayoutY() <= 576
					&& sprite.getLayoutY() >= 400)
					|| (sprite.getLayoutX() <= 978 && sprite.getLayoutX() >= 772 && sprite.getLayoutY() <= 862
							&& sprite.getLayoutY() >= 632)
					|| (sprite.getLayoutX() <= 1234 && sprite.getLayoutX() >= 1040 && sprite.getLayoutY() <= 862
							&& sprite.getLayoutY() >= 674)
					|| (sprite.getLayoutX() <= 1198 && sprite.getLayoutX() >= 1040 && sprite.getLayoutY() <= 576
							&& sprite.getLayoutY() >= 400)) {
			...
			// Interaction avec les cochons
			if (sprite.getLayoutX() <= 364 && sprite.getLayoutX() >= 196 && sprite.getLayoutY() <= 576
					&& sprite.getLayoutY() >= 380) {
				BoutonInteractionCochon.setVisible(true);
			} else {
				BoutonInteractionCochon.setVisible(false);
			}
			// Interaction avec le coffre
			if (sprite.getLayoutX() <= 1446 && sprite.getLayoutX() >= 1208 && sprite.getLayoutY() <= 242
					&& sprite.getLayoutY() >= 48) {
				BoutonInteractionCoffre.setVisible(true);
			} else {
				BoutonInteractionCoffre.setVisible(false);
			}
			// Interaction avec les vaches
			if (sprite.getLayoutX() <= 1668 && sprite.getLayoutX() >= 1512 && sprite.getLayoutY() <= 288
					&& sprite.getLayoutY() >= 0) {
			...
			if (wPressed.get()) {
				if ((((sprite.getLayoutX() < 470 && sprite.getLayoutY() <= 144)
						|| (sprite.getLayoutX() > 1130 && sprite.getLayoutY() <= 144) || sprite.getLayoutY() > 144))
						&& (-2 < sprite.getLayoutY())) {
			...
			if (aPressed.get()) {
				if (((sprite.getLayoutX() < 470 || 1138 < sprite.getLayoutX()) && sprite.getLayoutY() < 144
						&& -12 < sprite.getLayoutX()) || sprite.getLayoutY() > 144 && -12 < sprite.getLayoutX()) {
					System.out.println(" X : " + sprite.getLayoutX() + " Y : " + sprite.getLayoutY());
			...
			if (dPressed.get()) {
				if (((sprite.getLayoutX() < 464 || 900 < sprite.getLayoutX()) && sprite.getLayoutY() < 144
						&& 1832 > sprite.getLayoutX()) || sprite.getLayoutY() > 144 && 1832 > sprite.getLayoutX()) {
					System.out.println(" X : " + sprite.getLayoutX() + " Y : " + sprite.getLayoutY());
					sprite.setLayoutX(sprite.getLayoutX() + movementVariable);
				}
			}
	...
	/**
	 * Ouvre la page d'informations du personnage.
	 */
	@FXML
	...
	/**
	 * Ouvre la page d'inventaire du personnage.
	 */
	@FXML
	...
	/**
	 * Ajoute l'item "porc" à l'inventaire du joueur lorsqu'il interagit avec le
	 * bouton Cochon.
	 * 
	 * @param evt l'événement d'action du bouton.
	 */
	@FXML
	...
	/**
	 * Ajoute l'item "carotte" à l'inventaire du joueur lorsqu'il interagit avec le
	 * bouton Potager.
	 * 
	 * @param evt l'événement d'action du bouton.
	 */
	...
	/**
	 * Ajoute l'item "lait" à l'inventaire du joueur lorsqu'il interagit avec le
	 * bouton Lait.
	 * 
	 * @param evt l'événement d'action du bouton.
	 */
	...
	/**
	 * Ouvre la page du coffre si le joueur a suffisamment de coins, sinon affiche
	 * une alerte.
	 * 
	 * @param evt l'événement d'action du bouton.
	 */
	...
	/**
	 * Ouvre la page d'échange avec le marchand.
	 * @param evt l'événement d'action du bouton.
	 */
	@FXML
